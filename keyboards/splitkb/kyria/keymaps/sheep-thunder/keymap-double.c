...
uint8_t current_tap_frame[2] = {0,0};
...
static const uint8_t chunk_position[2][TOTAL_TAP_CHUNKS_RIGHT][2] = {
    ...
    // left
    {
        ...
        {0,2}, {0,3}, {4,1}, {4,2}, {8,1}, {8,2}, {10,3}, {12,2}, {14,3}, {14,4}, {18,2}, {18,3}, //7
...
static void render_thunder(uint8_t side) { 
    oled_clear();

    if (get_current_wpm() >= IDLE_SPEED) {
        // render the frame depending on WPM
        if (current_tap_frame[side] < IDLE_FRAMES) {
            // always render the first few frames
            for (int chunk_sub_idx; chunk_sub_idx < nb_of_chunks[side][current_tap_frame[side]]; chunk_sub_idx++) {
        ...
        current_tap_frame_idx[side] = (current_tap_frame_idx[side] + nb_of_chunks[side][current_tap_frame[side]]) % total_tap_chunks[side];
        current_tap_frame[side] = (current_tap_frame[side] + 1) % TAP_FRAMES;
        // start or keep looping
        looping_animation = true;
    } else {
        if (looping_animation) {
            if (current_tap_frame[side] == 0) {
                // we are done looping 
                looping_animation = false;
                finish_thunder_animation = false;
            } else if (current_tap_frame[side] < IDLE_FRAMES) {
